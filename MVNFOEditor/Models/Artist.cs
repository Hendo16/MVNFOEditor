...
public class Artist
{
    ...
    public static async Task<Artist> CreateArtist(string name)
    {
        ...
        //Just the artist object with no external metadata presumes a manually created artist
        newArtist.AddMetadata(new ArtistMetadata(newArtist));
        App.GetDBContext().Artist.Add(newArtist);
        await App.GetDBContext().SaveChangesAsync();
        return newArtist;
    }
    ...
    public async Task<Stream> LoadCardBannerBitmapAsync()
    {
        ...
        if (string.IsNullOrEmpty(CardBannerURL)) return File.OpenRead("./Assets/defaultBanner.png");
        
        var data = await NetworkHandler.GetFileData(CardBannerURL);
        if (data == null)
        {
    ...
    public Stream LoadLocalCardBannerBitmapAsync()
    {
        if (File.Exists(CachePath + "/cardBanner.jpg")) return File.OpenRead(CachePath + "/cardBanner.jpg");
        return File.OpenRead("./Assets/defaultBanner.png");
    }
    ...
    public async Task<Stream> LoadLargeBannerBitmapAsync()
    {
        ...
        if (string.IsNullOrEmpty(LargeBannerURL)) return File.OpenRead("./Assets/defaultBanner.png");

        var data = await NetworkHandler.GetFileData(LargeBannerURL);
        if (data == null)
        {
            ToastHelper.ShowError("Cover Error", "Couldn't fetch album artwork, please check logs");
            return File.OpenRead("./Assets/defaultBanner.png");

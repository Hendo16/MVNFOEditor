...
public partial class NewResultDialogViewModel : ObservableObject
{
    ...
    private async Task<AlbumResultsViewModel> AlbumResultsSetup(Artist artist,
        SearchSource source)
    {
    ...
    private async Task<VideoResultsViewModel?> VideoResultsSetup(Artist artist, SearchSource source, Album? album = null)
    {
    ...
    //TODO: Adapt this to uniquely link to event that's passed through externally from this dialog based on
    // the result handler that is provided. Should be generic as possible.
    [RelayCommand]
    ...
    private void SetupManual()
    {
        switch (CurrentContent)
        {
            case ArtistResultsViewModel artistList:
                ...
                break;
            case AlbumResultsViewModel albumList:
                //CurrentContent = new ManualAlbumViewModel(CurrArt);
                break;
            case VideoResultsViewModel videoList:
                break;
        }

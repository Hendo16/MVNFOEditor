...
public partial class MusicVideoListViewModel : ObservableObject
{
    ...
    {
        if (asc)
            SingleCards = new ObservableCollection<SingleViewModel>(SingleCards.OrderBy(a => a.Title));
        else
            SingleCards = new ObservableCollection<SingleViewModel>(SingleCards.OrderByDescending(a => a.Title));
    }
